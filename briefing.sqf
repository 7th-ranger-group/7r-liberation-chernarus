
_exe = player createDiaryRecord ["diary", ["Liberation Server Rules","
<br/>
<font size='18'>Hostile and Civilian Scores</font>
<br />
<br/>
When you are at an FOB you will have the resources overlay on the ride side of
 your screen. In there at the bottom there are two percentages that are <b>critically important</b> to how the game should be played:
<br />
<br />
* Exclamation Mark (!) is the hostility level, we need to keep it low.
 Beyond 60% we need to consider doing sidemissions to lower it.
<br />
* The House Mark is the civilian score, a negative score means hostile guerella forces and IEDs
on the roads. We want to keep this score to a healthy positive, hearts and minds folks!
<br />
<br />
<font size='18'>Urban Combat Rules</font>
<br />
<br />
* 1. Do not use any type of heavy weapons in urban areas.
<br />
* 1.1 That includes GL weapons, 50cals or higher.
<br />
* 2. Do not blow up IEDs, disarm them.
<br />
* 3. Do not blow up civilian buildings, we lose civ score.
<br />
* 4. Mark cleared buildings.
<br />
* 5. After the area is clear search for and treat wounded civilians. They are highlighted
 on your GPS in a red circled area.

<br/>
<font size='18'>General Rules</font>
<br />
* 6. If a player asks you to cease fire inside an FOB you need to immediately comply.
<br />

"]];

_exe = player createDiaryRecord ["diary", ["Community Server Rules","
<br/>
<font size='18'>Help New Members</font>
<br />
<br/>
7R offers a 24/7 dedicated community (Liberation or Antistasi) server for all of its members. One of the goals of this offering is to gradually train and ramp up new members into our squad’s way of operating and collaborating. So when you find new members on the community server:
<br />
<br />
* Talk to them, engage them in a conversation.
<br />
* Guide them through the rules and guidelines of the unit.
<br />
* Take them in your squad and behave more typical than you normally would (demonstrate how we operate).
<br />
<br />

<font size='18'>Follow your training</font>
<br />
<br />

If total anarchy is 0 (zero) and how 7R runs operations is 10, we should operate on the community server at a 7 or 8 level. That means:
<br />
<br />
* Form proper squads, decide on comms.
<br />
* Have a squad leader equipped with proper comms (152).
<br />
* Do not lonewolf.
<br />
* Use your training (bounding, peeling, CQB, etc).
<br />
<br />
<font size='18'>Communicate your intentions</font>
<br />
<br />
* Be responsive on long-range when on the server.
<br />
* Call out any actions that can impact other people’s operations.
<br />
* Mark cleared buildings on the map.
<br />
"]];


_exe = player createDiaryRecord ["diary", ["7R Rules","
<br/>
<font size='18'>General Rules</font>
<br/>
<br/>
1. Be respectul and behave maturely towards anyone at all times.
Racism, Sexism, Homophobia or any other kinds of discrimination are not accepted.
<br /><br />
2. Do not act in any way that could directly or indirectly cause any harm to this community.
This covers your behaviour towards non-members as well as promoting or committing criminal behaviour.
<br /><br />

3. Do not share any of our unit's resources with non-members.
This includes everything that cannot be publicly found on our website.
<br /><br />

4. Be active.
Participate in at least one op per week or leave a LOA on Discord.
<br /><br />

5. Use a pronounceable and consistent name across all platforms.
<br /><br />

<font size='18'>In Game Rules</font>
<br />
6. Do not cheat or use any unauthorised mods.
<br /><br />
7. Follow our procedures as outlined in our handbook and trainings.
You do not need to know the entire handbook, but everyone is expected to follow the procedures that we teach during the recruitment process and the various trainings.
<br /><br />

8. Follow and respect the in game chain of command.
<br /><br />
9. Do not harm or harass teammates and allied forces on purpose.
<br /><br />
10. Do not deliberately commit war crimes.
<br /><br />
This includes killing civilians or other non-combatants.

<br /><br />
11. Avoid breaking immersion.
<br /><br />
This includes not spoiling any knowledge you may have when replaying a mission or when respawning and coming in as reinforcements. Using the in game chat should be reserved to solving immediate technical problems. Do not share any information while being unconscious / dead.
<br /><br />

12. Follow our respawn policy as outlined in the handbook.
<br /><br />
<font size='18'>Recording Rules</font>
<br />
13. Prior to streaming you must announce it cleary to everyone involved.
<br /><br />
14. Recording outside of our operations requires the consent of the people present.
<br /><br />
15. If an NCO / Officer requests you not to stream or publish a recording you are to follow that.
<br /><br />
Penalties for not following these rules range from reprimand to ban from this unit and will be decided on a case by case decision by the officers and NCOs.
"]];
